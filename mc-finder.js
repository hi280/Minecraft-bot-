const mcdata = require('minecraft-data')("1.12"); //Keeping this as is for now.
...
exports.recipes = (bot, itemName, useTable=true)=>{
	...
	if (item) {
		if (useTable) {
			...
			let repices = bot.recipesAll(item, 1, craftingTable);
			return repices;
		} else {
			let repices = bot.recipesAll(item, 1, null);
			return repices;
		}
	} else {
		console.log('no item')
